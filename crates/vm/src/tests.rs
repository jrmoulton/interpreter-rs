...
fn string_literal() {
    ...
    match vm.run() {
        Ok(object) => {
            assert_eq!(object, "this is a string".to_string().into());
            let expected = expect_file!["../tests/expect_test_results/string_literal.txt"];
            expected.assert_eq(&format!("{vm:#?}"));
        },
        Err(e) => {
            eprintln!("{e}");
...
fn string_concat() {
    ...
    match vm.run() {
        Ok(object) => {
            assert_eq!(object, "this is a string and rope".to_string().into());
            let expected = expect_file!["../tests/expect_test_results/string_concat.txt"];
            expected.assert_eq(&format!("{vm:#?}"));
        },
        Err(e) => {
            eprintln!("{e}");
...
fn array_literal() {
    ...
    match vm.run() {
        Ok(object) => {
            assert_eq!(
                object,
                evaluator::object::ArrayWrapper::from(vec![2.into(), 5.into(), 12.into()]).into()
            );
            let expected = expect_file!["../tests/expect_test_results/array_literal.txt"];
            expected.assert_eq(&format!("{vm:#?}"));
        },
        Err(e) => {
            eprintln!("{e}");
...
fn array_index_op() {
    ...
    match vm.run() {
        Ok(object) => {
            assert_eq!(object, 2.into());
            let expected = expect_file!["../tests/expect_test_results/array_index_op.txt"];
            expected.assert_eq(&format!("{vm:#?}"));
        },
        Err(e) => {
            eprintln!("{e}");
...
#[test]
fn string_index_op() {
    let code: &'static str = r#" "this"[1] "#;
    let mut vm = new_vm(code);
    match vm.run() {
        Ok(object) => {
            assert_eq!(object, "h".to_string().into());
            let expected = expect_file!["../tests/expect_test_results/string_index_op.txt"];
            expected.assert_eq(&format!("{vm:#?}"));
        },
        Err(e) => {
            eprintln!("{e}");
